...
class Order(models.Model):
    ...
    def __str__(self) -> str:
        time_created = self.created_at.strftime("%Y-%m-%d %H:%M:%S")
        return f"<Order: {time_created}>"
...
class Ticket(models.Model):
    ...
    class Meta:
        constraints = [models.UniqueConstraint(
                       ...
                       name="unique_active_ticket_movie_session_row_seat"
                       )
                       ]
    ...
    def __str__(self) -> str:
        ...
        return (f"<Ticket: {self.movie_session.movie.title} "
                f"{time} (row: {self.row}, seat: {self.seat})>")
        ...
        if self.row > rows or self.row < 1:
            raise ValidationError({"row":
                                  [f"row number "
                                   ...
                                   f"(1, rows): "
                                   f"(1, {rows})"]})
        if self.seat > seats or self.seat < 1:
